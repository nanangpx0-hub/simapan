        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <p class="mb-4 text-sm text-gray-600">{{ __('Hanya manifest DRAFT yang dapat diubah.') }}</p>
                    <form method="POST" action="{{ route('document_manifests.update', $manifest) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="w-1/2">
                                ...
                                <select id="from_work_unit_id" name="from_work_unit_id" required class="mt-1 block w-full border rounded px-3 py-2">
                                    @foreach ($units as $unit)
                                        <option value="{{ $unit->id }}" @selected((string) old('from_work_unit_id', (string) $manifest->from_work_unit_id) === (string) $unit->id)>{{ $unit->code }} — {{ $unit->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="w-1/2">
                                ...
                                <select id="to_work_unit_id" name="to_work_unit_id" required class="mt-1 block w-full border rounded px-3 py-2">
                                    @foreach ($units as $unit)
                                        <option value="{{ $unit->id }}" @selected((string) old('to_work_unit_id', (string) $manifest->to_work_unit_id) === (string) $unit->id)>{{ $unit->code }} — {{ $unit->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
